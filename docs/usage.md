...
- `options` (Object, optional)
  - `defaultMode` (String): Default mode when no user preference is saved. Options: `'auto'`, `'sidepanel'`, `'window'`. Default: `'auto'`
  - `userAgent` (String): Custom user agent string for browser detection. Default: `navigator.userAgent`
...
- `browser`: Detected browser ('chrome', 'firefox', 'safari', 'edge', 'dia', 'unknown')
...
- **Chrome/Edge/Dia**: Attempts to use sidepanel, falls back to window if unavailable
