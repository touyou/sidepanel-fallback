(function (g, l) {
    ...
    createRadioGroup(e, t) {
      ...
      return (
        ...
        t.forEach(i => {
          ...
          ((d.textContent = i.label), n.appendChild(a), n.appendChild(d), s.appendChild(n));
        }),
        s
      );
    }
  ...
  const E = Object.freeze(
    Object.defineProperty(
      { __proto__: null, EVENTS: o, EventEmitter: S, createDebugEvent: T, createErrorEvent: b },
      Symbol.toStringTag,
      { value: 'Module' }
    )
  );
            ...
            try {
              ...
              for (const C of a.dependencies)
                if (!this.completedStages.has(C))
                  throw new Error(`Dependency ${C} not completed for stage ${a.name}`);
              const d = await this._runStageWithTimeout(a);
              ((s[a.name] = d),
                this.completedStages.add(a.name),
                t.mark(`stage-${a.name}-complete`));
            } catch (d) {
              if ((t.mark(`stage-${a.name}-error`), (i[a.name] = d), a.required))
                throw new Error(`Required stage ${a.name} failed: ${d.message}`);
              console.warn(`Optional stage ${a.name} failed:`, d);
            }
        return (
    ...
    getRecommendations() {
      ...
      const n =
        this.operations.length > 0
          ? this.operations.reduce((a, d) => a + d.duration, 0) / this.operations.length
          : 0;
      return (
        n > 10 &&
          e.push({
  ...
  class ae {
    ...
    async openPanel(e) {
      ...
      try {
        const s = await this.launcher.openPanel(t, e),
          i = _(s, { requestedMode: this.mode, effectiveMode: t, browser: this.browser });
        return (
        ...
        this.eventEmitter.emit(
          o.PANEL_OPEN_ERROR,
          b(s, 'openPanel', { path: e, mode: t, requestedMode: this.mode, browser: this.browser })
        );
    ...
    async withSettingsUI(e) {
      ...
      try {
        ...
        e.appendChild(s);
        const i = _({}, { browser: this.browser, currentMode: this.mode });
        return m(i, 'settings');
      } catch (s) {
